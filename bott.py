...
BOT_TOKEN = "8999778583:AAG48Zcivw53tOYZhMGZaS0K0blU1BNmeXI"
...
async def is_verified(update, context):
    ...
    # Referral reward only once after channel verification
    cur.execute("""
...
async def start(update, context):
    ...
    if not await is_verified(update, context):
        return
...
async def like_uid_handler(update, context):
    ...
    # Demo check again before creating order
    if plan_id == "demo":
...
async def admin_callback(update, context):
    ...
    if data == "admin_likes":
        ...
        for order_id, uid_user, game_uid, plan_id, amount in rows:
...
async def admin_text(update, context):
    ...
    # Referral reward
    if context.user_data.get("admin_ref_reward"):
    ...
    # CC description
    if context.user_data.get("cc_description"):
...
async def text_router(update, context):
    ...
    if text == "❤️ Buy Like":
        ...
        await check_uid(update, context)
...
async def callback_router(update, context):
    ...
    if data.startswith("admin_") or data.startswith("cc_"):
...
def main():
    ...
    if (
        not BOT_TOKEN
        or BOT_TOKEN == "PASTE_BOT_TOKEN_HERE"
    ):
